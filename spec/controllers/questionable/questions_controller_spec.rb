...
describe Questionable::QuestionsController do
  routes { Questionable::Engine.routes }
  
  before do
    @user = User.find_by alias: "testuser1"
    Questionable::Question.create!(title: "Test 1", content: "test 1 question", user_id: @user.id)
  ...
  it "posts questions#create" do
    ...
    question = Questionable::Question.find_by unique_id: "test-3"
  ...
  it "put questions#update" do
    ...
    expect(response.status).to eq(302)
    question = Questionable::Question.find_by unique_id: "test-2"
    response.should redirect_to(question_path(question))
  end
