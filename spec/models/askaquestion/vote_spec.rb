...
describe Askaquestion::Vote do
  before do
    ...
    @vote = Askaquestion::Vote.create!(:voteable_id => @question.id, :voteable_type => @question.class.to_s, :user_id => @user.id)
    @vote.should be_valid
    
    @user = User.find_by alias: "testuser3"
    @answer = Askaquestion::Answer.create!(question_id: @question.id, content: "test answer", user_id: @user.id)
    @answer.should be_valid
    
    user_ids = User.all.map {|user| user.id}
    user_ids.each do |user_id|
      Askaquestion::Vote.create!(:voteable_id => @answer.id, :voteable_type => @answer.class.to_s, :user_id => user_id)
    end
  end
